...
package at.saltyy.switchly.feature.scan
...
/**
 * Shared CameraX/ML Kit scanner for Switchly QR codes and managed barcodes.
...
class UnifiedScanActivity : AppCompatActivity() {
    ...
    /** Prefer a code that can actually trigger Switchly when several codes are visible. */
    ...
    private fun isActionable(candidate: Candidate): Boolean {
        ...
        return candidate.kind == ScanCodeStore.Kind.QR || ScanCodeStore.isStrictSwitchlyUri(candidate.raw)
    }
    ...
    private fun handleCode(raw: String, kind: ScanCodeStore.Kind) {
        ...
        val uri = raw.toUri()
        val isSwitchlyUri = uri.scheme.equals("switchly", ignoreCase = true)
        val validDirectAction = when (kind) {
            ScanCodeStore.Kind.QR -> isSwitchlyUri
            ScanCodeStore.Kind.BARCODE -> isSwitchlyUri && ScanCodeStore.isStrictSwitchlyUri(raw)
        }
