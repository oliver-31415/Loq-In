 ...
 * Full protection uses [LoqInAccessibilityService].
 * On Android 16 devices enrolled in Advanced Protection, Accessibility can be unavailable; in that specific case LoqIn may start a limited UsageEvents-based whole-app fallback.
...
object BlockingRuntime {
    ...
    // Called by [LoqInAccessibilityService] while it is connected and alive.
    fun markAccessibilityHeartbeat(ctx: Context) {
        ...
        runCatching { com.oliver.loqin.util.ProtectionStatusNotifier.onAccessibilityHeartbeat(ctx) }
    }
    ...
    fun ensureRunning(ctx: Context) {
        ...
        runCatching { com.oliver.loqin.util.ProtectionStatusNotifier.refresh(ctx) }
        runCatching { OemAccessibilityKeepAlive.sync(ctx) }
        runCatching { UsageAccessFallbackBlocking.sync(ctx) }
    }

    /**
     * Stops app-managed fallback runtime pieces when LoqIn is disabled.
     * Accessibility itself remains system-managed and is never toggled by LoqIn.
     */
    fun stop(ctx: Context) {
        // When LoqIn is turned off or temporarily disabled, clear stale blocker UI/state.
        ...
        runCatching { com.oliver.loqin.util.ProtectionStatusNotifier.refresh(ctx) }
        // sync() reads the real persisted LoqIn state, so transient UI flows cannot accidentally tear down a runtime that should still be active.
        runCatching { OemAccessibilityKeepAlive.sync(ctx) }
        runCatching { UsageAccessFallbackBlocking.sync(ctx) }
    }
}
