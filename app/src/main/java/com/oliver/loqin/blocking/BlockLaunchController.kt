...
internal class BlockLaunchController(
    ...
    fun showAppBlocker(pkg: String, label: String, delayMs: Long) {
        handler.postDelayed({
            if (!SwitchModeStore.isEnabled(service)) {
                ...
                BlockingRuntime.markBlockerVerify(service, pkg, "skipped reason=loqin_disabled")
                AppLogStore.append(service, "Blocking", "[blocker_launch] pkg=$pkg result=skipped reason=loqin_disabled")
                return@postDelayed
            }
    ...
    private fun launchAppBlocker(pkg: String, label: String, reason: String) {
        if (!SwitchModeStore.isEnabled(service)) {
            ...
            BlockingRuntime.markBlockerVerify(service, pkg, "launch_skipped reason=$reason loqin_disabled")
            AppLogStore.append(service, "Blocking", "[blocker_launch] pkg=$pkg result=skipped reason=loqin_disabled launchReason=$reason")
            return
        }
    ...
    private fun verifyAppBlockerVisible(pkg: String, label: String) {
        if (!SwitchModeStore.isEnabled(service)) {
            ...
            BlockingRuntime.markBlockerVerify(service, pkg, "verify_skipped loqin_disabled")
            AppLogStore.append(service, "Blocking", "[blocker_verify] pkg=$pkg result=skipped reason=loqin_disabled")
            return
        }
        ...
        handler.postDelayed({
            if (!SwitchModeStore.isEnabled(service)) {
                ...
                BlockingRuntime.markBlockerVerify(service, pkg, "retry_skipped loqin_disabled")
                AppLogStore.append(service, "Blocking", "[blocker_verify] pkg=$pkg result=retry_skipped reason=loqin_disabled")
                return@postDelayed
            }
