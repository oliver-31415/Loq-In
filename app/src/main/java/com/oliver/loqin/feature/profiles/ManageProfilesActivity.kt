...
import com.oliver.loqin.data.prefs.InAppLimitStore
...
class ManageProfilesActivity : AppCompatActivity() {
    ...
    private fun openAppListForProfile(profile: String) {
        val intent = Intent(this, AppPickerActivity::class.java).apply {
            ...
            putExtra(AppPickerActivity.EXTRA_ALLOW_LOCKED_PROFILE_STRICT_EDITS, true)
        }
    ...
    private fun showAddProfileSheet() {
        showProfileNameDialog(
            ...
            if (ProfileStore.addProfile(this, name)) {
                ...
                // Preserve the currently enforcing profile while protection is active.
                if (!isProfileLockActive()) {
                    ProfileStore.setCurrent(this, name)
                }
                refresh()
                snackRoot().showLoqInStatus(getString(R.string.profile_created_open_apps, name))
        ...
        showProfileNameDialog(
            ...
            when {
                ...
                newName == oldName -> {
                    ...
                    refresh()
                    snackRoot().showLoqInStatus(getString(R.string.profile_saved, oldName))
                    null
                }
                ProfileStore.renameProfile(this, oldName, newName) -> {
                    ProfileStore.setDescription(this, newName, description)
                    refresh()
                    snackRoot().showLoqInStatus(getString(R.string.profile_saved, newName))
                    null
                }
    ...
    private fun showProfileNameDialog(
        ...
        dialog.setOnShowListener {
            ...
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                if (isProfileLockActive() && !allowWhileLocked) {
                    dialog.dismiss()
                    syncProfileLockUi()
    ...
    private fun duplicateProfile(name: String) {
        ...
        if (ok) {
            ...
            snackRoot().showLoqInStatus(getString(R.string.profile_duplicated, newName))
            refresh()
        } else {
            snackRoot().showLoqInStatus(getString(R.string.profile_name_exists, newName))
        }
    }
    ...
    private fun setActiveProfile(name: String) {
        ...
        ProfileStore.setCurrent(this, name)
        snackRoot().showLoqInStatus(getString(R.string.profile_set_active_toast, name))
        refresh()
    }
    ...
    private inner class ProfileAdapter : RecyclerView.Adapter<ProfileAdapter.VH>() {
        ...
        inner class VH(view: View) : RecyclerView.ViewHolder(view) {
            ...
            private val actionState: ImageView = view.findViewById(R.id.ivProfileActionState)
            ...
            fun bind(name: String) {
                ...
                val limitedEditing = isProfileLockActive()
                val description = ProfileStore.getDescription(this@ManageProfilesActivity, name)
                subtitle.text = when {
                    limitedEditing && description.isBlank() -> getString(R.string.profile_limited_edit_row)
                    description.isBlank() -> getString(R.string.profile_row_subtitle)
                    else -> description
                }
                actionState.setImageResource(R.drawable.edit_24)
                actionState.contentDescription = getString(R.string.edit)
                actionState.alpha = 1f
                ...
                itemView.alpha = 1f
                // Profiles remain inspectable while protection is active. The action sheet exposes only
                // safe operations; destructive/profile-switching actions stay disabled until LoqIn is off.
