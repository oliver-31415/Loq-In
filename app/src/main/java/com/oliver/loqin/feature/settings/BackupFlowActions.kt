...
import com.oliver.loqin.data.sync.BackupCategory
import com.oliver.loqin.data.sync.BackupCategoryFilter
import com.oliver.loqin.data.sync.BackupSelection
import com.oliver.loqin.data.sync.BackupSelectionStore
import com.oliver.loqin.data.sync.FileBackupRuntime
import com.oliver.loqin.data.sync.inspectBackupCompatibility
...
class BackupFlowActions(private val activity: AppCompatActivity) {
    ...
    fun showBackupOptions() {
        ...
        val options = mutableListOf<LoqInDialogOption>()
        ...
        actions += ::fileRestore
        activity.showLoqInOptionDialog(
    ...
    private fun isRestricted(): Boolean = LoqInAppAccessGuard.isLocked(activity)
    ...
    private fun showBackupCategoryDialog(initial: BackupSelection, onSelected: (BackupSelection) -> Unit) {
        ...
        categoryDialog = ctx.showLoqInMultiChoiceDialog(
            ...
            if (selection.categoryIds.isEmpty()) {
                ...
                return@showLoqInMultiChoiceDialog
            }
    ...
    private fun defaultBackupFileName(): String {
        ...
        return "loqin-backup-$stamp.json"
    }
    ...
    private fun buildRestoreContentsPreview(payload: Map<*, *>): String {
        ...
        val internalPrefs = mapAt("loqin_prefs")
    ...
    private fun showRestoreSelectionChoices(
        ...
        restoreSelectDialog = ctx.showLoqInMultiChoiceDialog(
        ...
        if (!ok) {
            ...
            runCatching {
                ...
                try {
                    ...
                    ctx.getSharedPreferences("loqin_prefs", Context.MODE_PRIVATE).edit(commit = true) { clear() }
                    ctx.getSharedPreferences("loqin_prefs_schedules", Context.MODE_PRIVATE).edit(commit = true) { clear() }
                    ctx.getSharedPreferences("loqin_ui_hints", Context.MODE_PRIVATE).edit(commit = true) { clear() }
