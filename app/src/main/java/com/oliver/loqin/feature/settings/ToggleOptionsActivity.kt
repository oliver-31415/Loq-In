...
import com.oliver.loqin.widget.QrScanWidgetProvider
import com.oliver.loqin.widget.NextScheduleWidgetProvider
import com.oliver.loqin.widget.BlockedNotificationsWidgetProvider
import com.oliver.loqin.widget.BarcodeScanWidgetProvider
...
import com.oliver.loqin.feature.onboarding.QuickTileHelper
...
open class ToggleOptionsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fun canChangeControlMode(showToast: Boolean = true): Boolean {
            val allowed = !LoqInAppAccessGuard.isControlSettingsLocked(ctx)
            if (!allowed && showToast) {
                findViewById<View>(android.R.id.content)
                    .showWarnPill(getString(R.string.mode_switch_requires_loqin_disabled))
            }
            return allowed
        }
        ...
        fun applyControlModeSelection(mode: AutomationModeStore.Mode, userInitiated: Boolean) {
            ...
            // Control methods: how LoqIn can be enabled/disabled.
    ...
    private fun refreshLiveLockUi() {
        ...
        runCatching {
            ...
            val modeSwitchingAllowed = !LoqInAppAccessGuard.isControlSettingsLocked(this)
    ...
    private fun blockIfProtectionLockedForThisSection(): Boolean {
        return if (allowsControlSettingsRecovery()) {
            LoqInAppAccessGuard.blockControlSettingsIfLocked(this)
        } else {
            LoqInAppAccessGuard.blockIfLocked(this)
        }
    }
    ...
    private fun isMixedChannelEditingLocked(): Boolean {
        return LoqInAppAccessGuard.isControlSettingsLocked(this)
    }
    ...
    private fun refreshMixedChannelInteractivity() {
        ...
        // Quick Settings tile setup rows are safe to change while LoqIn is active.
