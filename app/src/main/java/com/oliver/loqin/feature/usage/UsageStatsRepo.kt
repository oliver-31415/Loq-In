...
object UsageStatsRepo {
    ...
    fun getTodaySummary(ctx: Context, topN: Int = 20): UsageSummary {
        // User-facing Today statistics should represent the whole device day, not only the time during which LoqIn protection happened to be enabled.
        // When Usage Access is available and this query is already running off the main thread, prefer Android's live usage data.
        // getSingleDayUsageByPackage() also merges LoqIn's local counter as a floor, which keeps the result useful on OEMs that publish UsageStats with a delay.
        // Important: profile/app-limit enforcement intentionally continues to use LoqIn's own counters. 
