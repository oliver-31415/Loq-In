...
// Loads and transforms app usage data.
object AppUsageRepo {
    ...
    fun getDaySummary(ctx: Context, timeMillis: Long, topN: Int = 8): UsageSummary {
        ...
        return if (local.isNotEmpty()) {
            buildSummary(ctx, local, topN, UsageSanity.RangeCap.OVERALL)
        } else {
    ...
    fun getLastNDaysSummary(ctx: Context, days: Int, topN: Int = 20): UsageSummary {
        ...
        return if (local.isNotEmpty()) {
            buildSummary(ctx, local, topN, UsageSanity.RangeCap.WEEK)
        } else {
            UsageSummary(0L, emptyList())
        }
    }
    ...
    fun getThisYearSummary(ctx: Context, topN: Int = 20): UsageSummary {
        ...
        return if (local.isNotEmpty()) {
            buildSummary(ctx, local, topN, UsageSanity.RangeCap.YEAR)
        } else {
            UsageSummary(0L, emptyList())
        }
    }
    ...
    private fun buildSummary(ctx: Context, raw: Map<String, Long>, topN: Int, rangeCap: UsageSanity.RangeCap): UsageSummary {
        ...
        while (it.hasNext()) {
            val pkg = it.next()
            if (UsageInsightsAppFilter.shouldHide(ctx, pkg) || !isInstalled(ctx, pkg)) {
                it.remove()
            }
        }
        if (byPkg.isEmpty()) {
