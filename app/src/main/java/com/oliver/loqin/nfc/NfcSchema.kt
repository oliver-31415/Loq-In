...
/**
 * Utility for building and parsing LoqIn NFC/deep-link URIs.
 * Canonical URI pattern written by LoqIn:
 *   loqin://<action>[?duration=<minutes|ask>][&profile=<profileName>]
 * Examples:
 *   loqin://toggle
 *   loqin://temp_disable?duration=15
 *   loqin://temp_enable?duration=ask
 *   loqin://temp_enable?profile=Work&duration=30
 * Additional readable/compact aliases remain supported for user-generated QR/barcode/NFC tags:
 *   loqin://action?type=temp_enable&profile=Work&duration=30
 *   loqin://a?t=te&p=Work&d=30
 * Legacy URI patterns remain supported for already-written NFC/QR/barcode tags:
 *   loqin://switch/<action>
 *   loqin://profile/<profileName>/<action>
 */
object NfcSchema {

    // Scheme
    private const val SCHEME = "loqin"

    // Hosts
    const val HOST_ACTION = "action" // Readable alias: loqin://action?type=...
    private const val HOST_COMPACT_ACTION = "a" // Compact alias: loqin://a?t=...
    ...
    /**
     * Strict parser for externally supplied LoqIn command URIs.
     ...
     *  - loqin://toggle
     *  - loqin://temp_disable?duration=10
     *  - loqin://temp_enable?duration=ask
     *  - loqin://temp_disable?duration=ask
     */
     ...
     *  - loqin://enable?profile=Work
     *  - loqin://temp_enable?profile=Home&duration=30
     *  - loqin://temp_enable?profile=Home&duration=ask
     *  - loqin://temp_disable?profile=Home&duration=ask
     */
