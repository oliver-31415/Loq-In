...
import com.oliver.loqin.data.prefs.NfcTempDisableLimiterStore
import com.oliver.loqin.data.prefs.NfcUidPairingStore
import com.oliver.loqin.data.prefs.QrTempActionLimiterStore
import com.oliver.loqin.data.prefs.ScanCodeStore
import com.oliver.loqin.data.prefs.ScanActionHistoryStore
 ...
 * Supported formats:
 *   Canonical: loqin://action?type=<action>[&duration=<minutes|ask>][&profile=<Profile>]
 *   Legacy: loqin://switch/<action> and loqin://profile/<Profile>/<action>
 */
class NfcEntryActivity : Activity() {
    ...
    private fun handleIncomingIntent(intent: Intent?) {
        ...
        // If Android delivers a manifest-dispatch intent during the tiny resume/reader-mode handoff window, swallow it here instead of treating the tag as a normal LoqIn action and showing "not linked".
        ...
        // In-app QR/barcode dispatches must carry a one-time token created by LoqIn.
        ...
        val writtenData = extractLoqInUri(intent)
        ...
        if (data == null || command == null) {
            if (fromNfc) {
                // UID-only paired tags are only used as a fallback for tags without a written LoqIn action.
                // Written loqin:// NFC action tags work without pairing as long as NFC control is allowed.
                ...
                val reason = when {
                    !pairedUidsEnabled -> "no_loqin_action_paired_tags_disabled"
                    pairedUids.isEmpty() -> "no_loqin_action_no_paired_tags"
                    seenUid.isBlank() -> "no_loqin_action_no_uid"
            ...
            val incomingLoqInUri = incomingUri?.scheme?.equals("loqin", ignoreCase = true) == true
            val reason = when {
                incomingLoqInUri -> "unsupported_action"
                incomingUri != null -> "invalid_payload"
                else -> "not_linked"
            }
            val message = when {
                incomingLoqInUri -> getString(R.string.scan_error_unsupported_action)
    ...
    private fun extractLoqInUri(intent: Intent?): Uri? {
    ...
    private fun handleProfileAction(
        ...
        when {
            ...
            // Profile disable should NOT disable LoqIn globally unless that profile is currently active.
            action in listOf("stop", "disable", "off") -> {
             ...
             * - If LoqIn is off -> enable and select this profile
             * - If LoqIn is on and this profile is active -> disable
             * - If LoqIn is on and another profile is active -> switch profile (keep enabled)
             */
            action == "toggle" -> {
                val enabled = SwitchModeStore.isEnabled(this)
                val current = ProfileStore.getCurrent(this)
