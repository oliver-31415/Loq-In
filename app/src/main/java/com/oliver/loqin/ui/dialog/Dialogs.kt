 ...
 * Colors resolve from the caller's theme — Theme.LoqIn maps all M3 roles to the
 ...
 */
fun Context.showLoqInInputDialog(
    title: CharSequence,
 ...
 */
fun Context.showLoqInInfoDialog(
    title: CharSequence,
...
/**
 * One shared LoqIn option-dialog style.
 ...
 */
private fun Context.showLoqInOptionDialogInternal(
    title: CharSequence,
    subtitle: CharSequence?,
    options: List<LoqInDialogOption>,
    ...
    val content = LinearLayout(this).apply {
        ...
        subtitle?.takeIf { it.isNotBlank() }?.let { subtitleText ->
            addView(TextView(this@showLoqInOptionDialogInternal).apply {
                text = subtitleText
...
fun Context.showLoqInOptionDialog(
    title: CharSequence,
    options: List<LoqInDialogOption>,
    onCancelled: (() -> Unit)? = null,
    compact: Boolean = false,
    ...
    confirmSelection: Boolean = options.any { it.selected },
    onSelected: (index: Int) -> Unit
): AlertDialog = showLoqInOptionDialogInternal(
    title,
    null,
    options,
    onCancelled,
...
fun Context.showLoqInMultiChoiceDialog(
    title: CharSequence,
    options: List<LoqInDialogOption>,
    ...
    val content = LinearLayout(this).apply {
        ...
        if (forceHorizontalButtons) {
            ...
            val footer = LinearLayout(this@showLoqInMultiChoiceDialog).apply {
                orientation = LinearLayout.HORIZONTAL
                gravity = Gravity.END or Gravity.CENTER_VERTICAL
                setPadding(0, dp(8), 0, dp(12))
            }
            inlineCancelButton = MaterialButton(this@showLoqInMultiChoiceDialog).apply {
            ...
            inlineConfirmButton = MaterialButton(this@showLoqInMultiChoiceDialog).apply {
...
fun Context.showLoqInOptionDialog(
    ...
    confirmSelection: Boolean = options.any { it.selected },
    onSelected: (index: Int) -> Unit
): AlertDialog = showLoqInOptionDialog(
    getString(titleRes),
    options,
    onCancelled,
    compact,
 ...
 */
fun AlertDialog.styleLoqInDialogButtons() {
    ...
    runCatching { applyLoqInDialogCorners() }
}
...
fun Context.styleLoqInFormButtons(
...
fun Context.showLoqInFormDialog(
    title: CharSequence,
    content: View,
    cancelButton: MaterialButton,
    saveButton: MaterialButton,
