...
object AppBlockSafety {
    ...
    /**
     * Packages that LoqIn must never block, even after a user confirms a protected-app warning.
    ...
    fun isAllowModeEssential(context: Context, pkg: String): Boolean {
        ...
        // LoqIn itself must never be blocked in Allow selected mode, otherwise the user can lock themselves out of the recovery UI.
        if (normalized == context.packageName) {
            return true
        }
