...
enum class BackupCategory(
    ...
    STRICT_PROTECTION(
        ...
        description = "Loq In app access lock, Device Admin uninstall protection and settings-bypass protection"
    ),
    STATISTICS(
        id = "statistics",
...
object BackupSelectionStore {
    private const val PREFS = "loqin_backup_selection"
...
object BackupCategoryFilter {
    ...
    fun filterPayloadForRestore(payload: Map<*, *>, selection: BackupSelection): Map<String, Any?> {
        ...
        val internalMap = filterInternalPrefs(stringKeyMap(payload[FIELD_LOQIN_PREFS]), selection)
        ...
        return mapOf(
            ...
            FIELD_LOQIN_PREFS to internalMap,
    ...
    private fun categoriesForDefaultPrefsKey(key: String): Set<BackupCategory> = when {
        ...
        key.startsWith("usage_limit_session_runtime__") ||
            ...
            key.startsWith("loqin_runtime_ms_") ||
            key.startsWith("loqin_active_") ||
