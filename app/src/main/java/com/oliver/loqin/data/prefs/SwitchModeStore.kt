...
object SwitchModeStore {
    ...
    private fun recordEffectiveStateChange(
        ...
        val action = if (enabledAfter) {
            LoqInActionCountStore.Action.ENABLE
        } else {
            LoqInActionCountStore.Action.DISABLE
        }
        LoqInActionCountStore.incrementToday(ctx, action)

        if (causedBySchedule) {
            val scheduleAction = if (enabledAfter) {
                LoqInActionCountStore.Action.SCHEDULE_ENABLE
            } else {
                LoqInActionCountStore.Action.SCHEDULE_DISABLE
            }
    ...
    /**
     * Permanently enables or disables LoqIn (user toggle).
    ...
    /**
     * Enables/disables LoqIn.
     * Security: if LoqIn is currently enabled and "require NFC to disable" is enabled, then calls that try to disable LoqIn will be ignored unless [allowNfcBypass] is true.
     */
    fun setEnabled(ctx: Context, enabled: Boolean, allowNfcBypass: Boolean): Boolean {
        ...
        if (currentlyEnabled != effectiveAfter) {
            ...
            // go through setEnabledBySchedule and never touch this.
            ScheduleRuntimeStore.setManualDisableMs(ctx, if (!effectiveAfter) now else 0L)
        }
    ...
    // Enable or disable LoqIn as triggered by schedules.
    fun setEnabledBySchedule(ctx: Context, enabled: Boolean) {
        ...
        // Hard rule:
        // NFC lock ON  -> schedules cannot disable LoqIn, but they may still enable it.
    ...
    // Temporarily disables LoqIn for the given duration in milliseconds.
    // Per-profile pause caps (count / per-pause max / daily total) are enforced here
    // so every entry point (sheet, NFC, widgets, settings) behaves the same.
    fun setTemporarilyDisabled(ctx: Context, durationMs: Long, isEmergency: Boolean = false): Boolean {
        val sp = ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        val now = System.currentTimeMillis()
        ...
        val profile = ProfileStore.getCurrent(ctx).orEmpty().trim().ifEmpty { "Default" }
        ...
        sp.edit {
            ...
            // If we flipped KEY_ENABLED=false, LoqIn would stay disabled after the timer expires.
        ...
        ActiveTimerWidgetProvider.updateAll(ctx)
        PersistentStatusNotifier.refresh(ctx)
        AppLogStore.append(ctx, "Profiles", "Temp disable started duration=${grantedMs}ms")
    ...
    fun setTemporarilyEnabled(
        ...
        val baseBefore = if (activeTempUntil > now && sp.contains(KEY_BASE_BEFORE_TEMP_ENABLE)) {
            ...
            // If LoqIn was genuinely enabled before starting Temporary Enable, expiry should keep it enabled.
            true
        } else {
    ...
    // Cancels an active temporary disable window and re-enables LoqIn immediately.
    ...
    // Returns whether NFC is required to disable LoqIn.
     ...
     * - While LoqIn is enabled, you may NOT turn this OFF (to avoid bypassing active lock).
    ...
    fun setNfcRequiredForDisable(ctx: Context, required: Boolean) {
        ...
        // While LoqIn is enabled, block attempts to disable the NFC requirement unless Emergency Bypass is active
