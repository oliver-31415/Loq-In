 ...
 * Apps hidden from app pickers are also excluded from LoqIn app protection so sensitive apps are not inspected through Accessibility or blocked by existing app/in-app rules while hidden.
 * Both sets live in loqin_prefs so they follow the existing local/cloud backup path.
 */
object IgnoredUsageAppsStore {
     ...
     * This keeps the existing two-list UI simple: an app hidden from LoqIn's app pickers is not inspected or blocked while it remains hidden.
     */
