# Loq In UI and interaction conventions
This document records the visual and interaction rules used across Loq In. Follow these conventions when adding or changing screens so navigation, actions, accessibility, and security behavior remain predictable.

## Core principles
- Prefer existing Loq In components and patterns over introducing screen-specific styling.
...
- Preserve access to intended unlock methods while Loq In is active, but do not expose tools that can create or reveal a bypass.
...
| --- | --- | --- |
| Opens another screen, submenu, or editor inside Loq In | Right chevron | `@drawable/keyboard_arrow_right_24` |
...
- Widget rows keep a right chevron when they open another Loq In configuration or explanation screen.
...
- Keep the primary action on the positive side and the neutral/cancel action on the negative side, following the existing Loq In dialog helpers.
...
- Rows that leave Loq In for Android settings or another app use `open_in_new`.
- Do not expose raw QR, barcode, NFC, deep-link, or command payloads in an informational popup while Loq In is active if they could be used to bypass protection.

## Active-state and security-sensitive UI
When Loq In is active:
...
- Recheck active state in `onResume()` for management screens that may remain open while Loq In becomes active.
...
- Every Kotlin source file keeps the Loq In GPL license header.
...
- Remove resources only after confirming they are unused across all configurations and generated references.
...
- active and inactive Loq In states
...
- screenshots for the pull request when the visual result changes

### Activity overview metrics
- App usage rows show **Opens** and **Blocks** for the selected range when data exists.
- Loq In overview uses the short labels **Opens** and **Blocks** consistently.
...
- Screen section headings use `@style/LoqIn.SectionHeader`; use 24dp before a new section and 10dp between its heading and first card.
