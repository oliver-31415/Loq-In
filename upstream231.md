...
| 2026-09-20 | **W5.2 UI round 2** | `feature/upstream-w5-protection-gate` | Done — `4ddd292` | Owner feedback: info moved to a toolbar overlay dialog; delay picker replaced by a discrete slider with snap points (Off → 7 days, locked-constrained, no typing); pending rows show the target app icon (globe for website rules); in-app rule keys mapped to display labels via `InAppRuleLabels` ("Mini player" instead of `block_yt_mini_player`) |
| 2026-09-20 | **W5.2 UI round 3** | `feature/upstream-w5-protection-gate` | Done — `33ffe27` | Owner feedback: slider lacked granularity → replaced with days/hours/minutes duration wheels (any value 1 min–7 days, Off switch, locked-constrained); queued feedback is now a bottom pill everywhere (no dialogs); per-item review is a bottom sheet and discard shows a pill; the picker marks queued unblocks with an amber tile + clock badge (`pendingPackagesProvider` in `AppListAdapter`, `ProtectionChangeGate.pendingAppSelectionPackages`) |
| 2026-09-21 | **W5.2 UI round 4** | `feature/upstream-w5-protection-gate` | Done — `cea737f` | Owner feedback: amber + clock badge replaced by a lighter shade of the accent and a faded check (picker), and the same pending treatment now applies to website rules (faded switch previewing the target state + "Pending — applies after the delay") and in-app rules (faded switch preview); wheels dialog cleaned up (no hint/switch, taller wheels, 0/0/0 = Off, locked hint only when invalid); re-queueing the same target keeps the original timer (`upsert` preserves executeAt) |
| 2026-09-21 | **W5.2 P4+P5** | `feature/upstream-w5-protection-gate` | Done — `dfd572d` | `ProtectionEditPolicy` deleted (website add uses a local stricter-only check); pending queue excluded from backups (`BackupExclusionTest`); delay key routed to CONTROL_SETTINGS; Support report shows delay + pending count |
| 2026-09-23 | **Removal drop fix** | `feature/upstream-w5-rest` | Done — `3cec1ff` | Root cause found with logging: the gate's APPLY_NOW branch applied only the **stricter half** of a selection change, so unchecking (a pure removal) wrote nothing while protection was off — exactly the "it does not remember" report. Now the full requested set is applied when unlocked. Verified on the emulator with the owner's prefs: unchecking Calendar removed it from the store |
| 2026-09-23 | **Auto-save root-cause fix** | `feature/upstream-w5-rest` | Done | Owner video: removals were not persisting. Root cause: the "reconcile pending with selection" call ran on harmless no-op auto-saves (including when the picker opens) and cancelled queued removals, because the store still contains the app until the timer. Reconcile removed from auto-save; undo now has its own path (`cancelPendingAppSelection` fired when a pending tile is tapped back to its stored state). Verified on the emulator: uncheck → queued; reopen picker → queue survives; tap again → cancelled |
| 2026-09-22 | **Pending target state** | `feature/upstream-w5-rest` | Done | Owner report: after navigating away and back, an app with a queued removal looked checked again ("auto-save didn't work"). The tile now renders the pending target state (unchecked + hourglass) on every bind, and tapping it undoes the queued change. Note: with the delay Off while protection is active, weakening toggles are still refused by design |
| 2026-09-22 | **Auto-save UX** | `feature/upstream-w5-rest` | Done | Owner report: removing an app looked broken because a queued removal re-baselined the list. Auto-save now keeps the intent on screen (tile unchecked + hourglass badge + pill, store unchanged until the timer), rebinds tiles after queueing, and cancels a queued removal when the app is re-checked (`reconcilePendingWithSelection`, `pruneAllowAdditionsNotIn`). Verified on the emulator: uncheck → "Chrome, not selected" + "Queued — will apply after the delay"; re-check → queue cancelled |
| 2026-09-22 | **Lock + badge rules** | `feature/upstream-w5-rest` | Done | Owner decisions: temporary pauses stay locked (a pause must not weaken rules), **Emergency Unlock counts as unlocked** so edits apply instead of queuing (`ProtectionChangeGate.isEditingUnlocked`, used by the gate and the picker/in-app pre-checks). Picker: the stopwatch badge is gone (blocked and limited both show the check; the "N min/day" subtitle states the limit) and the redundant dot stays hidden — only genuinely queued changes show the hourglass |
| 2026-09-22 | **Badge clarity** | `feature/upstream-w5-rest` | Done | Owner report: a limited app's clock badge read as "queued". Queued changes now use a distinct hourglass badge (plus the lighter shade); limited keeps the clock, blocked keeps the check |
| 2026-09-22 | **Picker auto-save** | `feature/upstream-w5-rest` | Done | Owner report: after "Remove limits" the badge stayed and the unselect needed Save. The picker now auto-saves every selection change through the gate (250 ms debounce, queued changes re-baseline the list, refusals revert), the Save button is hidden, limit removal rebinds the tile, and allow-mode Clear all asks before blocking everything. Verified on the emulator: unselect persisted without Save; the removed limit's badge disappeared |
| 2026-09-22 | **Limit removal prompt** | `feature/upstream-w5-rest` | Done | Unselecting an app that has limits now asks "Remove limits for <app>?" (Keep / Remove) with a "Remember my choice" checkbox; the default (Ask / Remove / Keep) is configurable in Feature access. Clearing limits is direction-aware: block mode selected -> stricter (hard block), unselected -> weaker; allow mode inverted (`limitClearDirection`, unit-tested) |
| 2026-09-22 | **Limit semantics fix** | `feature/upstream-w5-rest` | Done | Owner question clarified a real bug: a limit overrides a whole-app block (limited apps are allowed until the limit), so adding the first limit to a blocked app is a **weakening** and removing the last one is a **strengthening** — the gate had both backwards. Added `ProtectionChangePolicy.appLimitSetDirection` (boundary-aware), boundary handling in `requestAppLimits`, 9 unit tests, and a state line in the limit editor ("Fully blocked. Adding a limit allows it…") |
| 2026-09-22 | **Verification guide** | `docs/verification-guide.md` | Written | Full recreate-and-verify guide: setup, queue mechanics, per-surface tests, UI, W5.1/W5.4/W6 checks, emulator reproduction commands, and the four still-unverified items |
| 2026-09-22 | **Dialog polish** | `feature/upstream-w5-rest` | Done | Owner feedback: the new dialogs were ugly. Schedule save preview now uses the app's row-style info dialog (label/value rows + Save schedule; `showLoqInInfoDialog` gained an optional confirm callback), and the whole-app offer dialog gained an icon. Preview verified on the emulator |
| 2026-09-21 | **W6.5 E12 / W6.6 E9** | — | **Skipped** | W6.5: our bottom-nav menu/icons are part of the redesigned home; upstream's keep rule targets its own drawables. W6.6: onboarding was redesigned, upstream's adapter reorder conflicts |
| 2026-09-21 | **W6.4 E8** | `feature/upstream-w5-rest` | Done | Enabling an in-app rule for an app that is not whole-app managed now offers "Also block <app> entirely?"; confirming goes through `ProtectionChangeGate.requestAppSelection` (applies or queues), and the in-app rule stays active. Skipped the offer for protected/always-excluded apps |
| 2026-09-21 | **W6.3 E7** | `feature/upstream-w5-rest` | Done | `SchedulePreviewFormatter` (pure lines + Android renderer) covering all 7 of our `ScheduleStore.Action` values including `DISCONNECT_ENABLE/DISABLE`, Wi-Fi/Bluetooth/location (enter/exit/both), one-time/weekly, time range and note; save flow shows the preview dialog behind `AUTOMATION_SAVE_PREVIEW`; 8 new unit tests (79 total). UI check pending: save a schedule and confirm the preview appears |
| 2026-09-21 | **W6.2 E6** | `feature/upstream-w5-rest` | Done — `1631a72` | Map picker defers `SupportMapFragment` creation to a `post` on the container, reuses an attached fragment, attaches via `runOnCommit`, and only reports "map unavailable" on a real failure/timeout. Note: no `MAPS_API_KEY` is configured in this environment, so maps cannot load until `signing.properties`/gradle property/env supplies one |
| 2026-09-21 | **Support copy** | `feature/upstream-w5-rest` | Done — `d1a61f7` | Owner feedback: Info row still read "App logs" and mentioned contacting support. It is now "Copy support info" and copies the diagnostics report straight to the clipboard (pill, then returns); the report screen stays reachable for the full view |
| 2026-09-21 | **W5.4 D5** | `feature/upstream-w5-rest` | Done | Support report: canonical protection state, settings schema, feature flags, diagnostics timeline; Info → Support now opens the report (it previously opened the raw logs) |
| 2026-09-21 | **W5.3 C2** | — | **Skipped** | Owner decision: no separate Advanced Protection hub; the W5.2 page and the support report cover it |
| 2026-09-21 | **W5.1 D3** | `feature/upstream-w5-rest` | Done — `c418a8f` | `FeatureFlagStore` (4 flags: diagnostic timeline, schedule save preview, 2 scaffolds), developer tiles with live On/Off + scaffold markers, `AppLogStore` timeline feed gated on the timeline flag (default on). UI check pending: App info → long-press the version row → Feature flags |
| 2026-09-21 | **Finish plan** | `docs/upstream-finish-plan.md` | Written | Comprehensive plan to close out the port: W5.2 leftovers, W5.1, W5.3, W5.4, Phase 2 re-scoped (not upstream), W6.2–W6.6, verification backlog, push/PR strategy, open decisions and gaps |
| 2026-09-21 | **W5.2 limit fix 3** | `feature/upstream-w6-polish` | **Reverted** — `f007cfa` | Briefly pointed the Websites-list clock button at the compact limit editor, which dropped the "daily" wording. Owner: do not rework the website limits UI until after upstreaming; reverted. Phase-1 behavior stands: that button opens the rule editor (locked while active), while the limit editor from a website's usage detail is gated and works |
| 2026-09-21 | **W5.2 limit fix 2** | `feature/upstream-w6-polish` | Done — `1efe1c9` | Owner report: website limits still refused — `WebsiteUsageDetailActivity.btnEditLimits` had its own lock pre-check; removed. Follow-up requested: per-visit/opens limits for websites (see below) |
| 2026-09-21 | **W5.2 limit fix** | `feature/upstream-w6-polish` | Done — `b85d508` | Owner report: app/website limits still refused while active. The picker's limit button/long-press pre-check was removed (editor opens, gate decides on save) and website limits now go through a new `website_limit` request (apply/queue/deny with from-value guard, `websiteLimitDirection` unit-tested); verified 0→30 applies immediately while locked |
| 2026-09-21 | **W5.2 P6 + merge** | → `feature/upstream-w6-polish` | Done — fast-forward | Verified: in-app toggle queue, apply-now while fully off (applies + clears), due entry for a deleted profile dropped at startup, website/picker pending states, wheels. Remaining manual: limit dialog, auto-block, clear-all, remove-blocked-app, Doze timing, OEM alarms. W5.2 merged into the upstreaming line; `feature/upstream-w5-protection-gate` kept |
...
### W5.2 C1 — ProtectionChangeGate core — **COMPLETE 2026-09-21 (P0–P6)**
...
- **P3** (`9ca2b22`, `f9ac39b`): gate wired into `AppPickerActivity` (save with stricter/weaker delta split, row toggles, auto-block new apps, bulk clear via a new `clear_app_data` pending type), `AppListAdapter` (unavailable clears), `InAppRulesActivity` (surface toggles), `ManageBlockedWebsitesActivity` (rule enable/disable), `QuickLimitDialogs` (app limits), `MainActivity.removeBlockedApp` (selection + limit clear). Inactive-profile edits keep their existing bypass; website add/edit, website limits, rule-mode and control-mode/profile changes keep the structural deny (never queued). `ProtectionEditPolicy` is now unused except `canAddBlockedWebsite` (deleted in P4).
  ...
  - Queue fix (`161f927`, owner-reported): `app_selection` and `clear_app_data` dedupe keys now include the package list, so unblocking a second app no longer replaces the first entry or resets its timer; entries are per target and individually discardable, and labels name the apps ("Unblock Calendar"). Verified on the emulator: two unblocks produced two entries with independent 15-minute timers, and the page listed "Unblock Calendar" and "Unblock Chrome". A stricter edit also prunes matching pending weakenings (pure logic unit-tested; `pruneAppSelection`/`pruneInAppSelections`/`pruneWebsiteEnabled`/`pruneAutoBlock`).
